...
static int long_constant_instruction(const char *name, chunk_t *chunk, int offset) {
    uint8_t low_byte = chunk->code[offset + 1];
    uint8_t mid_byte = chunk->code[offset + 2];
    uint8_t hig_byte = chunk->code[offset + 3];
    uint32_t index = low_byte + (mid_byte << 8) + (hig_byte << 16);
